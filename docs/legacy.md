> Historical documentation for the pre-JAX implementation. Its simplex
> “BEING FIXED” warning is superseded by the current
> [simplex implementation and validation](simplex.md). The text below is
> preserved for historical reference.
> For executable examples of the current APIs, use the
> [current README](../README.md) and [task guide](usage.md). In particular,
> minimum-cost flow is exposed as `optinpy.mcfp.bigm`, and numerical objectives
> should use `jax.numpy`.
